...
router.post("/posticon", multer({dest: path.join(UpImgDirBase, "/icons"), limits:{fieldSize: 50}}).single('file') ,function(request, response) {
    // console.log(request)
    if (request.body.file) {
        if (request.user) {
            const userId = request.user.id;
            const currentIconFileName = request.user.icon_path ? request.user.icon_path : "";
            // request.user.icon_pathから、拡張子だけを取り出す
            const currentIconPath = path.join(UpImgDirBase, "/icons", currentIconFileName);
            console.log(currentIconPath)
            // 画像のタイプをBase64から取得
            //;base64の前までを取得
            const imgExt  = request.body.file.split("/")[1].split(";")[0];
            //画像はBase64で送られてくるので、デコードしてから画像を保存する
            const imgData = Buffer.from(request.body.file.replace(/^data:image\/\w+;base64,/, ""), 'base64');
            // console.log(imgExt);
            // console.log(imgData);
            if (imgData) {
                // 拡張子を取得
                // const ext = temp_path.split(".").pop();
                const dest = path.join(path.join(UpImgDirBase, "/icons"), uuidv4()) + "." + imgExt;
                // ファイル名を取得
                ...
                fse.writeFile(dest, imgData, (err) => {
                    if (err) {
                        ...
                        const icon_path: userTable["icon_path"] = "/images/icons/" + fileName;
