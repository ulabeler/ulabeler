...
const temp_password = <HTMLInputElement>(
  document.querySelector('input[name="temp_password"]')
);
const new_password = <HTMLInputElement>(
  document.querySelector('input[name="new_password"]')
);
const new_password_confirmation = <HTMLInputElement>(
...
new_password.addEventListener("keyup", function () {
  if (new_password.value.length < 8) {
    ...
    myXOR(new_password.value.match(/[A-Z]/), new_password.value.match(/[a-z]/))
  ) {
...
function resetPasswordAttempt() {
  ...
  if (check_password_confirm()) {
    // @ts-ignore
    axios
      .post("/api/user/reset_password_attempt", {
        id: id,
        token: token,
      ...
      .then(function (response: { status: number; data: string }) {
        ...
        if (response.status == 201) {
          ...
          response.status == 200 &&
          response.data == "Temp Password is wrong"
        ) {
