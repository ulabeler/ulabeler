...
const InputEmailConfirm = <HTMLInputElement>(
  document.getElementById("input_email_confirm")
); // <%# メールアドレス(確認用) %>
const InputPassword = <HTMLInputElement>(
  document.getElementById("input_password")
); // <%# パスワード %>
const InputPasswordConfirm = <HTMLInputElement>(
  document.getElementById("input_password_confirm")
); // <%# パスワード(確認用) %>
...
// エラー関係の表示用変数
const ErrorUsername = <HTMLInputElement>(
...
const ErrorPassword = <HTMLInputElement>(
...
InputUserName.addEventListener("focusout", function () {
  if (DisableCounterUsername == 0 && InputUserName.value.length > 0) {
    // @ts-ignore
    axios
      .post("https://tools.na2na.dev/api/word/is_includeNgWord", {
